...
uint32 IR_AdcResult[ADC_CHN_MAX];
...
void BasicVadcBgScan_run(void)
{
    ...
    volatile Ifx_VADC_RES conversionResult;
        ...
        for (chnIx = 0; chnIx < ADC_CHN_MAX; ++chnIx)
        {
			...
			IR_AdcResult[chnIx] =conversionResult.B.RESULT;

        }
}

boolean ObstacleDetected(void){
	if(IR_AdcResult[IR_SENSOR_PIN] >= OBSTACLE_DETECT_STANDARD)
		return TRUE;
	else return FALSE;
}
